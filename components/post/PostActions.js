...
class PostActions {
  ...
  render() {
    ...
    postActions.className = 'post-actions-post';    // Creiamo il pulsante upvote con contatore cliccabile per mostrare i votanti
  ...
  /**
   * Aggiorna lo stato visivo del pulsante reblog
   * @param {boolean} hasReblogged - Indica se l'utente ha rebloggato il post
   */
  updateReblogState(hasReblogged) {
    this.hasReblogged = hasReblogged;
    
    // Cerca il pulsante di reblog nel DOM
    const reblogBtns = document.querySelectorAll('.reblog-btn');
    reblogBtns.forEach(reblogBtn => {
      // Controlla se questo è il pulsante del nostro post
      if (reblogBtn) {
        // Aggiorna la classe CSS
        if (hasReblogged) {
          reblogBtn.classList.add('reblogged');
        } else {
          reblogBtn.classList.remove('reblogged');
        }
        
        // Aggiorna il testo
        const textSpan = reblogBtn.querySelector('span:last-child');
        if (textSpan) {
          textSpan.textContent = hasReblogged ? 'Reblogged' : 'Reblog';
        }
      }
    });
    
    console.log(`Reblog state updated: hasReblogged=${hasReblogged}`);
  }
